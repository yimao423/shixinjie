...
function _measureKeyboardHeight() {
  try {
    ...
    if (vv && vv.height && ih && vv.height > 0 && vv.height < ih) {
      return Math.max(120, Math.round(ih - vv.height));
    }
  } catch (e) {}
  return 0;
}
...
function _captureKeyboardHeight() {
  ...
  if (h > 0) _chatPanelKeyboardH = h;
}

/* 将面板高度变量应用到页面（无键盘时回退 300px） */
