...
var FAVORITE_CATEGORIES = [
  ...
  { key: 'sticker', label: '表情包', icon: 'fas fa-face-smile' },
  { key: 'image', label: '图片', icon: 'fas fa-image' },
  { key: 'doodle', label: '涂鸦', icon: 'fas fa-pen-nib' },
  { key: 'voice', label: '语音', icon: 'fas fa-microphone' },
  { key: 'redpacket', label: '红包', icon: 'fas fa-envelope-open' },
...
function _favoriteItemHtml(fav, readonly, mode) {
  ...
  if (fav.category === 'gift' && fav.giftData) {
    ...
    contentHtml = '<div class="favorite-decision">'
      ...
      + (d.pending
          ? '<div class="favorite-decision-status pending">等待对方作答</div>'
          : (d.answers && d.answers.length ? '<div class="favorite-decision-status done"><i class="fas fa-check-circle"></i> 已互答完成</div>' : ''))
      + '</div>';
  } else if (fav.category === 'redpacket' && fav.rpData) {
    // 红包收藏：还原红包记录（含领取态表现状）
    var rp = fav.rpData;
    var rpStatus = rp.claimed
      ? '<span class="favorite-rp-status claimed">已领取</span>'
      : (rp.returned ? '<span class="favorite-rp-status returned">已退回</span>' : '<span class="favorite-rp-status waiting">未领取</span>');
    contentHtml = '<div class="favorite-redpacket">'
      + '<div class="favorite-rp-head">'
      + '<span class="favorite-rp-icon">' + (rp.rpType === 'exclusive' ? '🧧' : (rp.rpType === 'lucky' ? '🎋' : '🧧')) + '</span>'
      + '<div class="favorite-rp-titles">'
      + '<div class="favorite-rp-name">' + Core.escapeHtml(rp.greeting || '恭喜发财') + '</div>'
      + '<div class="favorite-rp-sub">' + (rp.rpType === 'exclusive' ? '专属红包' : (rp.rpType === 'lucky' ? '拼手气红包' : '普通红包')) + '</div>'
      + '</div>'
      + '</div>'
      + '<div class="favorite-rp-foot">'
      + '<span class="favorite-rp-amount">¥' + (rp.totalAmount != null ? rp.totalAmount : 0) + '</span>'
      + rpStatus
      + '</div>'
      + '</div>';
  } else if (fav.category === 'voice' && fav.voiceData) {
    // 语音收藏：还原语音图标、时长与转文字内容
    var vd = fav.voiceData || {};
    var dur = vd.duration || 0;
    contentHtml = '<div class="favorite-voice">'
      + '<div class="favorite-voice-main"><i class="fas fa-volume-up favorite-voice-icon"></i>'
      + '<span class="favorite-voice-wave"><span></span><span></span><span></span><span></span><span></span></span>'
      + '<span class="favorite-voice-duration">' + Math.round(dur) + '"</span></div>'
      + '<div class="favorite-voice-meta">语音消息' + (vd.audioData ? '' : '（音频数据不可用）') + '</div>'
      + (vd.voiceText ? '<div class="favorite-voice-text">' + Core.escapeHtml(vd.voiceText) + '</div>' : '')
      + '</div>';
