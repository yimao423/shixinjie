...
const PartnerFreeWill = {
  ...
  /* ---- 他的收藏：仅收藏我方（用户）在角色聊中真实发送过的消息 ----
     绝不从字卡/拍一拍/兜底语料等站内内容池随机生成，对方角色自己发的内容一律不被收藏。
     无真实消息可收藏时（当前没有可收藏的真实消息 / 已全部收藏过）返回 null，由调用方静默跳过、不产生任何"他收藏"条目。 */
  _pickFavoriteSource() {
    return this._pickMyRealMessage();
  },
  ...
  _actHisFavorite(ts) {
    try {
      ...
      if (!src) return; // 无可收藏真实消息时静默跳过：不产生任何"他收藏"条目
      var msg = src.msg;
      var from = '对方';
      // 取当前对方角色名作为收藏来源，更自然
      try {
      ...
      var fav = {
        ...
        category: msg.msgType === 'doodle' ? 'doodle' : (msg.msgType === 'image' ? 'image' : 'text'),
        label: msg.msgType === 'doodle' ? '涂鸦' : (msg.msgType === 'image' ? '图片' : '文本'),
        text: msg.msgType === 'text' ? String(msg.text || '').trim() : (msg.msgType === 'doodle' ? '[涂鸦]' : '[图片]'),
        stickerData: msg.msgType === 'doodle' ? (msg.stickerData || '') : '',
        imageData: msg.msgType === 'image' ? (msg.imageData || '') : '',
        from: from,
        time: ts,
        isPartnerFree: true
      };
      // 真实消息溯源：附 chatId/msgId/srcIsReal，供「他的收藏」页删除/清空时反向清除聊天消息标记
      fav.chatId = src.chatId;
      fav.msgId = String(msg.id);
      fav.srcIsReal = true;
      // 与已有条目去重：按消息 id 判定，避免同一条真实消息重复收藏
      var hisFavs = Storage.getHisFavorites();
      var isDup = false;
      for (var i = 0; i < hisFavs.length; i++) {
        if (String(hisFavs[i].msgId) === String(fav.msgId)) { isDup = true; break; }
      }
      if (isDup) return;
      hisFavs.unshift(fav);
      Storage.setHisFavorites(hisFavs);
      // 把"被他悄悄收藏"标记写回聊天消息，供其气泡下方回显标签（持久化，刷新后仍保持）
      try {
        var chatMsgs = Storage.getMessages(src.chatId) || [];
        for (var k = 0; k < chatMsgs.length; k++) {
          if (String(chatMsgs[k].id) === String(msg.id)) {
      ...
      if (Math.random() < 0.35) {
        var brief = String(fav.text).length > 12 ? String(fav.text).slice(0, 12) + '…' : String(fav.text);
