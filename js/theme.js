...
const ThemeManager = {
  ...
  init() {
    ...
    if (!this.themes.find(function(t) { return t.id === this.currentTheme; })) {
    ...
    Promise.all([Storage.getAsync('theme'), Storage.getAsync('customThemeColor')]).then(function(vals) {
      ...
      if (theme && self.themes.find(function(t) { return t.id === theme; })) {
        // 先记录恢复前的同步生效值，用于判定恢复后是否需要重新 apply
        var prevTheme = self.currentTheme;
        var prevColor = self.customColor;
        self.currentTheme = theme;
        if (theme === 'custom' && color) self.customColor = color;
        ...
        //       却永不重绘，直至手动刷新镜像回填后下一会话才命中。这里强制 apply(true) + 写回镜像，
        //       使首次直接打开链接即正确显示自定义主题色（背景 + splash），无需手动刷新。
        var needApply = theme !== prevTheme || (theme === 'custom' && color && color !== prevColor);
        if (needApply) self.apply(true);
