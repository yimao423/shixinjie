...
function insertEmojiToInput(emojiChar) {
  ...
  onChatInputChange();
  // 记录最近使用（去重、最新在前、最多9个），并刷新「最近使用」分区
  if (Storage.getRecentEmojis) {
    ...
    if (recent.length > 9) recent = recent.slice(0, 9);
...
function scrollChatToBottom() {
  ...
  // 移动端软键盘弹出/收起过程中容器高度持续变化（--chat-h 单值跟随可视区），
  // 兜底时点拉长到 1100ms，覆盖键盘布局稳定期，避免新消息"卡半屏"需手动滑动。
